...
function renderCustomServices() {
    ...
    for (const [id, svc] of entries.sort((a, b) => a[1].name.localeCompare(b[1].name))) {
        ...
        const domainPreview = domains.slice(0, 5).map(d =>
            '<span class="text-xs font-mono text-gray-500 bg-gray-50 px-1 rounded">' + d + '</span>'
        ).join('');
        ...
        html += '<div class="px-6 py-4">' +
            ...
            '<span class="text-xs text-gray-400 ml-1 font-mono">(' + id + ')</span>' +
            '<span class="text-xs text-gray-400 ml-2">' + domains.length + ' domains</span>' +
            '</div>' +
            '<div class="flex space-x-2">' +
            '<button onclick="openServiceModal(\'' + escapedId + '\')" class="text-sm text-indigo-600 hover:text-indigo-500">Edit</button>' +
            '<button onclick="deleteService(\'' + escapedId + '\')" class="text-sm text-red-600 hover:text-red-500">Delete</button>' +
            '</div></div>' +
